...
char str_pres[100] = ""; // Formatted message to display the pressure value
uint8_t msg3[] = "=====> Pressure sensor lps22hb initialized \r\n ";
char str2[50];
float temp_value = 0;
float hum_value = 0;// Measured temperature value
char str_tmp[100] = ""; // Formatted message to display the temperature value
uint8_t msg4[] = "=====> Temperature sensor HTS221 initialized \r\n ";
...
int main(void)
{
  ...
  HAL_UART_Transmit(&huart1,msg3,sizeof(msg3),1000);
   if (BSP_PSENSOR_Init() != PSENSOR_OK)
     {
         // Optional: Send an error message if init fails
         char err_msg[] = "Sensor Init Error\r\n";
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  float temp_value = BSP_TSENSOR_ReadTemp();  // ← SAME SENSOR!
	   // Debug both values
	  sprintf(str2, "Temperature:%.1fC Humidity:%.0f%%\r\n", temp_value, hum_value);
	  HAL_UART_Transmit(&huart1, (uint8_t*)str2, strlen(str2), 1000);
	  HAL_Delay(1000);
	  p_value = BSP_PSENSOR_ReadPressure();
	  sprintf(str_tmp, "Pressure:%.1f\n\r", p_value);
	  HAL_UART_Transmit(&huart1, (uint8_t*)str_tmp, strlen(str_tmp), 1000);  // Use strlen()
	  HAL_Delay(1000);
