...
static WIFI_Status_t SendWebPage(uint8_t ledIsOn, uint8_t temperature)
{
  ...
  char html[2048];   // Bigger buffer for modern UI

  // Build HTML (Futuristic UI)
  ...
  "<title>STM32 Smart Control</title>"
  ...
  ".temp { font-size:28px; margin:20px; }"
  ...
  ".on { background:#00ffcc; color:black; }"
  ...
  "<h1>STM32 Smart Dashboard</h1>"

  "<div class='temp'>Temperature: %d&deg;C</div>"

  "<div class='status'>LED Status: %s</div>"

  "<form method='POST'>"
  "<button class='btn on' name='radio' value='1'>LED ON</button>"
  "<button class='btn off' name='radio' value='0'>LED OFF</button>"
  "</form>"
  ...
  // Send response
  ret = WIFI_SendData(SOCKET, (uint8_t*)html, strlen(html), &SentDataLength, WIFI_WRITE_TIMEOUT);

  if ((ret == WIFI_STATUS_OK) && (SentDataLength != strlen(html)))
  {
